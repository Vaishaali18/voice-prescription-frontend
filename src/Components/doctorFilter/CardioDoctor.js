...
export default function App()
{
  ...
  const { userData, setUserData } = useContext(UserContext);
    ...
    useEffect(()=>{
        ...
        const getDetails = async () => {
            const res = await Axios.get("https://voice-prescription-ai.herokuapp.com/doctor/viewCardio"); 
            setItems(res.data);
        }
        getDetails();
     },[])
  ...
  <div class="collapse navbar-collapse" id="navbarNavAltMarkup" style = {{'margin-left' : '30%'}}>
    <div class="navbar-nav">
      <a class="nav-item nav-link active" href="/admin/view/cardiologist" style={{'margin-right' : '3vw'}}>Cardiologist <span class="sr-only">(current)</span></a>
      <a class="nav-item nav-link" href="/admin/view/pediatrician" style={{'margin-right' : '3vw'}}>Pediatrician</a>
