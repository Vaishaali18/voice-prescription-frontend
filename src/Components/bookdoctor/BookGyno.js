...
export default function App()
{
  ...
  useEffect(()=>{
      ...
      const getDetails = async () => {
          const res = await Axios.get("https://voice-prescription-ai.herokuapp.com/users/bookGyno"); 
          setItems(res.data);
      }
     ...
     //   console.log(token)
        const tokenRes = await Axios.post("https://voice-prescription-ai.herokuapp.com/tokenIsValid", null, { headers: { "x-auth-token": token } });
    ...
    <div class="collapse navbar-collapse" id="navbarNavAltMarkup" style = {{'margin-left' : '30%'}}>
      <div class="navbar-nav">
        <a class="nav-item nav-link extra" href="/patient/book/cardiologist" style={{'margin-right' : '3vw'}}>Cardiologist <span class="sr-only">(current)</span></a>
        <a class="nav-item nav-link extra" href="/patient/book/pediatrician" style={{'margin-right' : '3vw'}}>Pediatrician</a>
        <a class="nav-item nav-link active" href="/patient/book/gynecologist" style={{'margin-right' : '3vw','color':'white'}}>Gynecologist</a>
